...
import { removeIndents } from './strings.js';
...
export async function saveImage({prompt = '', path = '', doRemoveBackground = false, width = 1024, height = 1024, saveAsJpg = false, contextForErrorLogging = "Dall-E", model = "dall-e-3", imageInfo = {revisedPrompt: null, temporaryLiveUrl: null}} = {}) {
  // API Reference: https://platform.openai.com/docs/api-reference/images/create
  ...
  try {
    ...
    if (saveAsJpg) {
      await saveImageAsJpg(imageUrl, path);
    }
    else {
